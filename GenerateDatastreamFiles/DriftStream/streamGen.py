...
import sys
from .SyntheticWoodsmoke.windSimStream import WindSimGenerator
#from moaLink.moaLink import get_moa_stream_string
import copy
import numpy as np
...
class AGRAWALConcept(Concept):
    def __init__(self, concept_id = 0, seed = None, noise = 0):
        self.cf = concept_id
        self.seed = seed
        stream = AGRAWALGenerator(concept_id, random_state = seed, perturbation = noise)
...
class TREEConcept(Concept):
    def __init__(self, concept_id = 0, seed = None, noise = 0, desc = None):
        ...
        stream = RandomTreeGenerator(tree_random_state=seed, sample_random_state=seed, max_tree_depth=self.difficulty+2, min_leaf_depth=self.difficulty, n_classes=2)
        stream.prepare_for_use()
        super().__init__(stream)
...
class SEAConcept(Concept):
    def __init__(self, concept_id = 0, seed = None, noise = 0):
        self.cf = concept_id
        self.seed = seed
        stream = SEAGenerator(classification_function = concept_id, random_state=seed)
        stream.prepare_for_use()
        super().__init__(stream)
...
class SINEConcept(Concept):
    def __init__(self, concept_id = 0, seed = None, noise = 0):
        self.cf = concept_id
        self.seed = seed
        stream = SineGenerator(classification_function = concept_id, random_state=seed)
        stream.prepare_for_use()
        super().__init__(stream)
...
class STAGGERConcept(Concept):
    def __init__(self, concept_id = 0, seed = None, noise = 0):
        stream = STAGGERGenerator(classification_function = concept_id, random_state=seed)
        self.cf = concept_id
        self.seed = seed
        stream.prepare_for_use()
        super().__init__(stream)
...
class WindSimConcept(Concept):
    ...
    def activate(self):
        self.datastream.set_concept(self.seed)
...
class RecurringConceptStream:
    def __init__(self, rctype, num_samples, noise, concept_chain, seed = None, desc = None, boost_first_occurance = True):
        ...
        if type(concept_chain) is dict:
            ...
            for i in concept_chain:
                ...
                if desc == None or cID not in desc:
                    examples_in_concept = num_samples / (num_concepts)
                else:
            ...
            # num_concepts = len(concept_chain)
            # if desc == None and 
            # examples_in_concept = num_samples / (num_concepts)
            # for i in range(len(concept_chain)):
            #     self.concept_chain[math.floor(i * examples_in_concept)] = concept_chain[i]
        
        self.concepts_used = sorted(list(np.unique(np.array(list(self.concept_chain.values())))))
        ...
        if self.rctype == RCStreamType.AGRAWAL:
            ...
            gen_func = lambda i, desc: AGRAWALConcept(i, self.random_seed + i)

        if self.rctype == RCStreamType.TREE:
            self.num_concepts_availiable = 100
            gen_func = lambda i, desc: TREEConcept(i, self.random_seed + i, desc = desc)

        if self.rctype == RCStreamType.RBF:
            self.num_concepts_availiable = 100
            gen_func = lambda i, desc: RBFConcept(i, self.random_seed + i, desc = desc)

        if self.rctype == RCStreamType.SEA:
            self.num_concepts_availiable = 4
            gen_func = lambda i, desc:  SEAConcept(i, self.random_seed + i)

        if self.rctype == RCStreamType.SINE:
            self.num_concepts_availiable = 4
            gen_func = lambda i, desc:  SINEConcept(i, self.random_seed + i)

        if self.rctype == RCStreamType.STAGGER:
            self.num_concepts_availiable = 3
            gen_func = lambda i, desc: STAGGERConcept(i, self.random_seed + i)

        if self.rctype == RCStreamType.WINDSIM:
            self.num_concepts_availiable = 10000
            gen_func = lambda i, desc: WindSimConcept(i, self.random_seed + i, desc = desc)
        ...
        if self.num_concepts_used > self.num_concepts_availiable:
            raise ValueError("Generator does not have enough concepts for the given concept chain")
    ...
    def has_more_samples(self):
        return self.concepts[self.current_concept].has_more_samples() and (self.example_count < self.num_concepts)

    def get_drift_info(self):
        return self.concept_chain
    
    def has_more_samples(self):
        return self.example_count < self.num_samples
    ...
    def get_moa_stream_info(self):
        ...
        return get_moa_stream_string(concepts)
...
class RecurringConceptGradualStream(RecurringConceptStream):
    def __init__(self, rctype, num_samples, noise, concept_chain, window_size = 1000, seed = None, desc = None, boost_first_occurance = True):
        ...
        super().__init__(rctype, num_samples, noise, concept_chain, seed = seed, desc = desc, boost_first_occurance = boost_first_occurance )
