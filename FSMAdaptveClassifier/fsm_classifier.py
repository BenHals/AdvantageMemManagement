...
class FSMClassifier:
    ...
    sensitivity = 0.05,
 run_non_active_states = False, concept_chain = None, optimal_selection = False, optimal_drift = False, rand_weights = True, poisson = 6,
 similarity_measure = 'KT', merge_strategy = "sur", use_clean = True, merge_similarity = 0.9, drift_detector = 'adwin'):
    ...
    def _partial_fit(self, X, y, sample_weight):
        ...
        # Experimental! Enable reverting to a new model if a reused
        # model is not performing well. Needs experimentation.
        # signal_backtrack = self.fsm.get_state().signal_backtrack
        signal_backtrack = False
        if signal_backtrack:
