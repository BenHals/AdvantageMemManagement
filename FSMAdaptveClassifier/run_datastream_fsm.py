...
import subprocess
import copy
import evaluate_prequential
from fsm_classifier import FSMClassifier
from skmultiflow.data import DataStream
from adaptiveLearner.tracksplit_hoeffding_tree import TS_HoeffdingTree
from adaptiveLearner.tracksplit_HAT import TS_HAT
from adaptiveLearner.TS_ARFTREE import TS_ARFHoeffdingTree
...
def get_TS_Hoef_tree(max_size):
    def l():
        return TS_HoeffdingTree(max_byte_size = max_size, memory_estimate_period = 1000)
    return l
    
def get_naive_bayes_learner():
    def l():
        return NaiveBayes()
    return l

def get_ARF_learner(n_estimators = 5):
    def l():
        return AdaptiveRandomForest(n_estimators= n_estimators)
    return l

def get_TS_HAT_learner():
    def l():
        return TS_HAT()
    return l

def get_ARF_HAT_learner():
    def l():
        return get_ARF_HAT()
    return l

def get_HC_learner(max_size, w):
    def l():
        return TS_HoeffdingTree(max_byte_size = max_size, memory_estimate_period = 1000, split_confidence= pow(0.0000001,  1 / w), grace_period= int(200 / w))
    return l

class learner_obj:
    def __init__(self, learner_type, max_size, w):
    ...
    def __call__(self):
        l = TS_HoeffdingTree(max_byte_size = self.max_size, memory_estimate_period = 1000)
...
if __name__ == "__main__":
    ...
    ap.add_argument("-cls", "--conceptlimitstart", type=int,
        help="Concept limit", default=1)
    ap.add_argument("-cpus", "--cpus", type=int,
    ...
    ap.add_argument("-mm", "--memmanagement",
        help="learner", default="rA", choices=["rA", "age", "LRU", 'acc', 'auc', 'score', 'all', 'div', 'mine'])
    ap.add_argument("-dd", "--driftdetector",
        help="learner", default="adwin", choices=["adwin", "EDDM", "DDM"])
    ...
    ap.add_argument("-uo", "--useold", action="store_true",
        help="use old fsm class")
    ap.add_argument("-nc", "--nochain", action="store_true",
        help="Don't look for a concept chain")
    ap.add_argument("-mgs", "--mergesim", type=float,
        help="merge similarity", default=0.9)
    args = vars(ap.parse_args())
    options = FSMOptions()
    options.drift_detector = args['driftdetector']
    options.max_size = args['maxsize']
    print(f"Max byte size: {options.max_size}")
    options.learner_str = args['adaptivelearner']
    options.learner = get_TS_Hoef_tree(options.max_size)
    if args['adaptivelearner'] == 'NBN':
        options.learner = get_naive_bayes_learner()
    if args['adaptivelearner'] == 'ARFN':
        options.learner = get_ARF_learner
    if args['adaptivelearner'] == 'HATN':
        options.learner = get_TS_HAT_learner()
    if args['adaptivelearner'] == 'ARF_HATN':
        options.learner = get_ARF_HAT_learner()
    if args['adaptivelearner'] == 'HC':
        w = args['poisson']
        options.learner = get_HC_learner(options.max_size, w)
    ...
    options.use_clean = not args['useold']
    options.merge_similarity = args['mergesim']
                ...
                for subdir in list_of_directories:
                    ...
                    for s in sensitivities:
                        for w in windows:
                            options.sensitivity = s
                            options.window = w
                            tests.append(copy.deepcopy(options))
                            # start_run(options)
    ...
    if args['cpus'] == 1:
        ...
        results = pool.map(start_run, tests)
